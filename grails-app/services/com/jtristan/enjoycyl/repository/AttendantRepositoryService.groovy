...
import com.jtristan.enjoycyl.Attendant
...
class AttendantRepositoryService {

    @Transactional
    def save(Attendant attendant) {
        attendant.statistic = Statistic.findByEventId(attendant.eventId)
        if (!attendant.region) attendant.region = attendant.locality.description
        attendant.save()
    }
